...
class ElfFile(StructBase):
    ...
    def pack_into(self, block, offset=0):
        ...
        #print ("size of block:{0:x}".format(len(block)))
        # currently the block is empty

        total, scoff, shoff, pcoff, phoff = self._offsets(offset)
        #print ("[+] ELF offsets : total={0:x},scoff={1:x},shoff={2:x},pcoff={3:x},phoff={4:x}".format(total,scoff,shoff,pcoff,phoff))
        print ("[+] ELF offsets : scoff={0},shoff={1},pcoff={2},phoff={3}".format(scoff,shoff,pcoff,phoff))
        ...
        #if self.fileHeader.type == ET.byname['ET_EXEC'].code:
        #  # Executables (EXEC) get patched in text segments.
        #  self._pack_segments(block, pcoff)
        #elif self.fileHeader.type == ET.byname['ET_REL'].code:
        #  # DLLs (REL) have already got patched in text sections.
        #  pass

        if len(self.sectionHeaders) == 0:
          # if we don't have sections, then patch content would be in in segments.
          self._pack_segments(block, pcoff)

        self._pack_program_headers(block, phoff)
        #print( hexdump.dump (block))
    ...
    def _offsets(self, offset=0):
        ...
        * sectionHeaders
        ...
        # segment starts at offset of .text segment
        try:
          x = self.programHeaders[0].offset
          pcoff = x
          pcsize=0
          for ph in self.programHeaders:
            pcsize+=ph.filesz
        except IndexError:
          # no segment headers exist
          pcoff = 0

        # section starts at offset of .text section
        try:
          x = self.sectionHeaders[1].offset
          scoff = x
          max_off=0
          for sh in self.sectionHeaders:
            if sh.type == SHT.byname['SHT_NOBITS'].code:
              continue
            #x += sh.section_size
            if sh.offset > max_off:
              max_off = sh.offset
              x=sh.offset + sh.section_size
          shoff = x
          x += (len(self.sectionHeaders) * self.fileHeader.shentsize)
        except IndexError:
          # no section headers exist
          scoff = 0
          shoff = 0
    ...
    def _pack_program_headers(self, block, offset):
        ...
        #load2_filesz=0
        #load2_memsz=0
        #ph=self.programHeaders[1]
        #for sh in self.sectionHeaders:
        #  if "rodata" in sh.name or "data" in sh.name or "bss" in sh.name:
        #    if sh.type != SHT.byname['SHT_NOBITS'].code:
        #      load2_filesz+=sh.section_size
        #    load2_memsz+=sh.section_size
        #ph.filesz=load2_filesz
        #ph.memsz=load2_memsz
            
        #print ("load2 filesize:{0:x}".format(load2_filesz))

        #print (self.section_changes)
        phiter = zip(range(0, len(self.programHeaders)), self.programHeaders)
        for i, ph in phiter:
            #print ("segment:{0}, offset:{1:x}".format(ph.type, ph.offset))
            #ph.offset = self.section_changes[ph.offset]
...
class ElfSectionHeader(StructBase):
    ...
    def pack_into(self, block, offset=0):
        ...
        if self.type == SHT.byname['SHT_NOBITS'].code:
          #hexdump.hexdump(self.content)
          return 
